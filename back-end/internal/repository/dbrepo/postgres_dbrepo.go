...
func (m *PostgresDBRepo) AllMovies(genre ...int) ([]*models.Movie, error) {
	...
	if len(genre) > 0 {
		genreFilter = fmt.Sprintf("where id in (select movie_id from movies_genres where genre_id = %d)", genre[0])
	}
...
func (m *PostgresDBRepo) UpdateMovie(movie models.Movie) error {
	...
	query := `update movies set title=$1, description=$2, release_date=$3, runtime=$4, mpaa_rating=$5, updated_at=$6, image=$7 where id=$8`
